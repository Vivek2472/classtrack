...
window.ClassTrackAuthView = {
  ...
  renderLogin(container) {
    const savedEmail = localStorage.getItem('classtrack_remember_email') || '';
    const isRemembered = Boolean(savedEmail);
            ...
            <form id="auth-login-form" class="flex flex-col gap-4" onsubmit="window.ClassTrackAuthView.handleLoginSubmit(event)">
              ...
              <div class="flex flex-col gap-1.5">
                ...
                <div class="relative">
                  ...
                  <input id="login-email" type="email" name="email" required value="${savedEmail}" placeholder="you@example.com" class="form-input font-body-md" style="padding-left: 42px; height: 46px; border-radius: 10px;">
                </div>
              </div>
              ...
              <div class="flex items-center gap-2 mt-0.5">
                <input id="login-remember" type="checkbox" name="remember" ${isRemembered || !savedEmail ? 'checked' : ''} class="w-4 h-4 rounded text-primary focus:ring-primary cursor-pointer">
  ...
  async handleLoginSubmit(e) {
    ...
    try {
      ...
      const email = (formData.get('email') || formData.get('identifier') || '').trim();
      const password = formData.get('password');
      const isRemember = formData.get('remember') !== null;

      if (isRemember && email) {
        try {
          localStorage.setItem('classtrack_remember_email', email);
        } catch (err) {}
      } else {
        try {
          localStorage.removeItem('classtrack_remember_email');
        } catch (err) {}
      }

      const res = await window.ClassTrackAuth.login(email, password);
      if (res.success) {
      ...
      if (res.success) {
        ...
        if (res.alreadyExists) {
          window.ClassTrackApp.confirmDialog({
            ...
            message: `An account with ${email} is already registered. Would you like to log in instead?`,
            icon: 'account_circle',
          ...
          window.ClassTrackApp.showToast(res.error, 'warning');
        } else {
          window.ClassTrackApp.showToast(res.error, 'error');
        }
      }
  ...
  async handleSocialLogin(provider) {
    const supabase = window.ClassTrackAuth?.getSupabase();
    if (!supabase) {
      window.ClassTrackApp.showToast('Cloud service is initializing. Please try again.', 'error');
      return;
    }
    try {
      ...
      if (error) {
        window.ClassTrackApp.showToast(error.message, 'error');
      }
    } catch (err) {
      window.ClassTrackApp.showToast(err.message || 'Social login failed.', 'error');
    }
  },
