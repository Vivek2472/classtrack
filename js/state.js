/**
 * ClassTrack Engineering - In-Memory State & Calculation Engine
 * Synchronized live with Supabase PostgreSQL (Zero local storage dependency)
 */
...
class StateManager {
  constructor() {
    ...
    // Fast synchronous cache restore on page refresh to eliminate "Student" placeholder flicker
    try {
      const cached = localStorage.getItem('classtrack_state_cache');
      if (cached) {
  ...
  saveState() {
    try {
      localStorage.setItem('classtrack_state_cache', JSON.stringify(this.state));
    } catch (e) {}
    this.notify();
  }
  ...
  /* ----------------------------------------------------
     Subject Management (Persisted to Supabase)
  ----------------------------------------------------- */
  ...
  /* ----------------------------------------------------
     Schedule Management (Persisted to Supabase)
  ----------------------------------------------------- */
