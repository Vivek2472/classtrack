...
window.ClassTrackApp = {
  ...
  init() {
    ...
    // Check initial route & authentication
    ...
    // "Try as Guest" CTA from landing page: automatically initialize guest session and open dashboard
    if (isGuest === '1') {
      window.ClassTrackAuth.loginAsGuest();
      this.updateProfileBadges();
    ...
    if (!window.ClassTrackAuth.isAuthenticated()) {
      // If user landed directly on index.html with no auth hash and no CTA param,
      // redirect to the public landing page first.
      if (!fromLanding && !authViews.includes(hash)) {
        window.location.replace('landing.html');
        return;
      }
      if (authViews.includes(hash)) {
  ...
  navigate(viewName, params = null, isBack = false) {
    ...
    if (!isAuthView && !window.ClassTrackAuth.isAuthenticated()) {
      viewName = 'login';
    }
    ...
    // Toggle auth-mode body class for full-screen auth layout vs app layout
    if (isAuthView) {
  ...
  updateProfileBadges() {
    ...
    const program = authUser?.branch || state.profile.program || 'Academic Profile';
    ...
    studentPrograms.forEach(el => el.textContent = isGuest ? `${program} (Guest)` : program);
  },
        ...
        <form id="form-profile" onsubmit="window.ClassTrackApp.submitProfile(event)">
          <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div class="form-group">
              ...
              <input type="text" name="name" class="form-input font-semibold" value="${state.profile.name || ''}" placeholder="e.g. Alex Morgan" required>
            </div>
            <div class="form-group">
              ...
              <input type="text" name="rollNo" class="form-input font-mono uppercase" value="${state.profile.rollNo || ''}" placeholder="e.g. STD-2024">
            </div>
          </div>
          ...
          <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div class="form-group">
              ...
              <input type="text" name="program" class="form-input" value="${state.profile.program || ''}" placeholder="e.g. Computer Science, Business, Arts">
            </div>
            <div class="form-group">
          ...
          <div class="form-group">
            ...
            <input type="email" name="email" class="form-input" value="${state.profile.email || ''}" placeholder="student@example.com">
          </div>
  ...
  submitProfile(e) {
    ...
    const profile = {
      ...
      program: formData.get('program') || 'General Studies',
    ...
    // Update currentUser in Auth if active
    if (window.ClassTrackAuth && window.ClassTrackAuth.session && window.ClassTrackAuth.session.user) {
      window.ClassTrackAuth.session.user.fullName = profile.name;
      window.ClassTrackAuth.session.user.universityId = profile.rollNo;
      window.ClassTrackAuth.session.user.branch = profile.program;
      window.ClassTrackAuth.session.user.semester = profile.semester;
      window.ClassTrackAuth.session.user.email = profile.email;
      window.ClassTrackAuth.saveSession(window.ClassTrackAuth.session);
    }
      ...
      <div class="p-6">
        ...
        <div class="flex flex-col gap-4">
          <div class="p-4 rounded-xl" style="background-color: var(--color-surface-container-low); border: 1px solid var(--color-outline-variant);">
            <h4 class="font-headline-sm font-bold mb-1">Export JSON Backup</h4>
            <p class="font-body-sm mb-3" style="color: var(--color-on-surface-variant);">Download your entire academic profile, schedule, and logs as a JSON backup file.</p>
            <button class="btn btn-secondary btn-sm" onclick="window.ClassTrackApp.downloadBackupJSON()">
              <span class="material-symbols-outlined">download</span> Download Backup (.json)
            </button>
          </div>
  ...
  downloadBackupJSON() {
    const data = JSON.stringify(window.EduTrackState.getState(), null, 2);
  ...
  clearAllData() {
    this.confirmDialog({
      ...
      message: 'This action will wipe all enrolled courses, weekly timetable schedules, and attendance history logs permanently.',
      icon: 'delete_sweep',
      confirmText: 'Clear All Data',
      confirmClass: 'btn-danger',
      onConfirm: () => {
        window.EduTrackState.resetToDefault();
        this.showToast('All data cleared. Fresh start!', 'info');
      }
    });
  },
